...
package in.illimitable.handlebar.lexer;
 ...
 * @author illimitable
 */
public class HBSLexer implements Lexer<HBSTokenId> {
    
    private static final int EOF = LexerInput.EOF;
    
    private final LexerInput input;
    
    private final TokenFactory<HBSTokenId> tokenFactory;
    ...
    public HBSLexer(LexerRestartInfo<HBSTokenId> info) {
    ...
    private Token<HBSTokenId> token(HBSTokenId id) {
        if(input.readLength() == 0) {
            new Exception("Error - token length is zero!; state = " + state).printStackTrace();
        }
        Token<HBSTokenId> t = tokenFactory.createToken(id);
        return t;
    }
    
    @Override
    public Token<HBSTokenId> nextToken() {
        int actChar;
        while (true) {
            actChar = input.read();
...
//                            return token(HBSTokenId.EOL);
                        ...
                        case '%': {
                            ...
                            if (input.readLength() == 1) {
                                state = ISI_JS_LINE;
                                return token(HBSTokenId.DELIMITER);
                            }
...
//                            break;
                    ...
                    switch (actChar) {
                        case '=': 
                            if(input.readLength() == 3) {
                                ...
                                // HBS symbol, but we also have content language in the buffer
                                input.backup(3); //backup <%=
                                state = INIT;
                                return token(HBSTokenId.HTML); //return CL token
                            }
                        case '%': {
                            int peek = input.read();
                            if (peek != LexerInput.EOF) {
                        ...
                        default:  // HBS scriptlet delimiter '<%'
                            if(input.readLength() == 3) {
                                // just <% + something != [=,#] read
                                state = ISI_SCRIPTLET;
                    ...
                    switch(actChar) {
                        case '>':
                            if(input.readLength() == 2) {
                                ...
                                input.backup(2); // backup '%>' we will read JUST them again
                                state = ISI_EXPR_SCRIPTLET;
                                return token(HBSTokenId.JS_EXPR);
                            }
                        default:
                            state = ISI_EXPR_SCRIPTLET;
                            break;
                    ...
                    switch(actChar) {
                        case '>':
                            if(input.readLength() == 2) {
                                ...
                                input.backup(2); // backup '%>' we will read JUST them again
                                state = ISI_COMMENT_SCRIPTLET;
                                return token(HBSTokenId.JSCOMMENT);
                            }
                        default:
                            state = ISI_COMMENT_SCRIPTLET;
                            break;
        ...
        switch(state) {
            ...
            default:
                System.out.println("HBSLexer - unhandled state : " + state);   // NOI18N
        }
